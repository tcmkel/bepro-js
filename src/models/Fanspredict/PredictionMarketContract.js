...
class PredictionMarketContract extends IContract {
  ...
  async getMarketDetails({ marketId }) {
    ...
    if (events.length === 0) {
      ...
      return {
        name: '', category: '', subcategory: '', image: '', outcomes: [],
      };
    }
  ...
  /**
	 ...
	 */
  async createMarket({
    name, image, duration, oracleAddress, outcomes, category, ethAmount,
  }) {
  ...
  /**
	 ...
	 */
  async buy({
    marketId, outcomeId, ethAmount, minOutcomeSharesToBuy,
  }) {
    const ethToWei = Numbers.toSmartContractDecimals(ethAmount, 18);
  ...
  /**
	 ...
	 */
  async sell({
    marketId, outcomeId, ethAmount, maxOutcomeSharesToSell,
  }) {
